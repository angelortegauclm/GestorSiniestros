...
def lambda_handler(event, context):
    ...
    message_text = f"Estimado usuario, su factura ha sido generada.\n" \
                   f"La factura '{file_key}' está disponible en el bucket '{bucket_name}'."
    
    try:
        response = sns_client.publish(
            TopicArn=SNS_TOPIC_ARN,
            Message=message_text,
            Subject='GESTOR SINIESTROS: Factura Generada'
        )
        print("Notificación de factura enviada con éxito:", response)
